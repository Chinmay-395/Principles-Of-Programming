...
(* The helper_function_find *)
let rec func3: string -> class_env -> 'a list =
  fun c_name c_env ->
match List.assoc_opt c_name c_env with
  | None -> []
  | Some (super_class_name,_field_names,_method_names) -> 
      super_class_name :: func3 super_class_name c_env

let rec find_string value list =
match list with
| [] -> false
